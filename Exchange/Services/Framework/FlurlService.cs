...
namespace Kadevjo.Core.Services
{
		...
		where T : Model
	{
		...
		protected async override Task<GenericResponse<R>> Execute<R>(string resource, IQuery query = null)
		{
			...
			try
			{
				Task<HttpResponseMessage> task = url.WithClient(client).GetAsync();
			...
			{
				return default(GenericResponse<R>);
			}
		}
		...
		protected async override Task<GenericResponse<R>> Execute<R, B>(
			string resource, HttpMethod method, B body = default(B), IQuery query = null)
		{
